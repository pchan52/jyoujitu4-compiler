/* XCC: Experimental C-subset compiler.
  ...
  $Id: xcc.y,v 1.4 2009/03/13 05:12:54 gondow Exp $ */ 
%{
#include <stdio.h>
#include <stdarg.h>
#include <stdlib.h>
...
expression
    : identifier
	  ...
	  $$->type = sym->type; }
    | INTEGER_CONSTANT
	{ $$ = create_AST ("AST_expression_int", 1, atoi (yytext));
	  $$->type = types.t_int; }
    | CHARACTER_CONSTANT
	{ 
          char c;
	...
	{ struct Symbol *sym;
          struct AST *ast;
          ast = create_AST ("AST_expression_id", 1, $1);
          sym = sym_check_var_decl (ast);
	  ast->type = sym->type;
          $$ = create_AST ("AST_expression_funcall1", 2, ast, $3);
	  $$->type = ast->type->u.t_function.ret_type;
	  if ($3->u.arg_size > max_arg_size)
	      max_arg_size = $3->u.arg_size; }
    | identifier '(' ')'
	{ struct Symbol *sym;
          struct AST *ast;
          ast = create_AST ("AST_expression_id", 1, $1);
          sym = sym_check_var_decl (ast);
	  ast->type = sym->type;
          $$ = create_AST ("AST_expression_funcall2", 1, ast);
	  $$->type = ast->type->u.t_function.ret_type; }
    | '(' expression ')'
...
argument_expression_list
	...
	{ $$ = create_AST ("AST_argument_expression_list_single", 1, $1);
	  $$->u.arg_size = ROUNDUP_INT ($1->type->size); }
    | argument_expression_list ',' expression
	{ $$ = create_AST ("AST_argument_expression_list_pair", 2, $1, $3);
	$$->u.arg_size = $1->u.arg_size + ROUNDUP_INT ($3->type->size); }
    ;
