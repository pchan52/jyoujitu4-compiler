...
static void codegen_expression_if(struct AST *ast);     //if文
static void codegen_expression_elseif(struct AST *ast); //elseif文
static void codegen_expression_return(struct AST *ast); //return文
...
visit_AST_right (struct AST *ast)
{
  ...
  } else if (!strcmp (ast->ast_type, "AST_expressioin_mul")){  // *式
    codegen_expression_mul (ast);
  } else if (!strcmp (ast->ast_type, "AST_expression_div")){  // /式
    codegen_expression_div (ast);
  } else if (!strcmp (ast->ast_type, "AST_expression_if")){   //if文
    codegen_expression_if (ast);
  } else if (!strcmp (ast->ast_type, "AST_expression_elseif")){ // elseif文
    codegen_expression_elseif (ast);
  } else if (!strcmp (ast->ast_type, "AST_expression_return")){ // return文
    codegen_expression_return (ast);
...
visit_AST_left(struct AST *ast) {
  int i;
  if(!strcmp (ast->ast_type, "AST_expressionid")){
    codegen_expression_id_left (ast);
  }else {
    for (i=0; i < ast->num_child; i++){
      visit_AST_right (ast->child[i]);
...
codegen_function_definition (struct AST *ast)
{
  ...
  //局所変数のサイズ分スタックに積む->スタックトップに変数を積むため
...
//if文
static void
codegen_expression_if (struct AST *ast)
{
  visit_AST_right(ast->child[0]);
  emit_code(ast, "\tpopl\t%%eax\n");
  frame_height -= 4;
...
//elseif文
static void
codegen_expression_elseif (struct AST *ast)
{
   visit_AST_right(ast->child[0]);
   emit_code(ast,"\tpopl\t%%eax\n");
   frame_height -= 4;
   emit_code(ast,"\tcmpl\t$0,%%eax\n");
   int label_elseif = label;
   label += 2;
   emit_code(ast,"\tje\tL%d",label_elseif);
   visit_AST_right(ast->child[1]);
   emit_code(ast,"\tjmp\tL%d\n",label_elseif+1);
   emit_code(ast,"L%d:\n",label_elseif);
...
//return 文
static void
codegen_expression_return (struct AST *ast)
{
  if(!strcmp(ast->child[0]->ast_type,"AST_expression_opt_single")){
    visit_AST_right(ast->child[0]);
    emit_code(ast,"\tpopl\t%%eax\n");
